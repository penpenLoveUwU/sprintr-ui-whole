...
class Map extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (prevProps.rawMap.metaData.nonce !== this.props.rawMap.metaData.nonce) {
       ...
       */
      if (document.visibilityState !== "visible" || this.activeTouchEvent || this.activeScrollEvent) {
  ...
  updateState() {
    ...
    const updatedSelectedSegmentIds = [...previouslySelectedSegmentIds.filter(id => {
  ...
  onScroll(evt) {
    ...
    } = this.ctxWrapper.getScaleFactor();
    if (factor * currentScaleFactor < 0.4 && factor < 1 || factor * currentScaleFactor > 150 && factor > 1) {
      return;
    }
  ...
  moveTranslate(evt) {
    const {
      x,
      y
    } = this.relativeCoordinatesToCanvas(evt.x1, evt.y1);
    ...
    if (this.touchHandlingState.dragStart) {
      ...
      const wasHandled = this.structureManager.getClientStructures().some(structure => {
        const result = structure.translate(this.touchHandlingState.dragStart.matrixTransform(invertedCurrentTransform), {
          x: oldX,
          y: oldY
        }, {
          x,
          y
        }, currentTransform, currentPixelSize);
        if (result.stopPropagation) {
          if (result.deleteMe === true) {
