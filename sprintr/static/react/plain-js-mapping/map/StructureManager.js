...
class StructureManager {
  ...
  updateMapStructuresFromMapData(rawMap) {
    this.mapStructures = [...this.buildMapStructuresFromEntityMapData(rawMap.entities), ...this.buildMapStructuresFromLayerMapData(rawMap.layers)];
    ...
    entities.forEach(e => {
      switch (e.type) {
        case RawMapEntityType.RobotPosition:
          {
            const p0 = this.convertCMCoordinatesToPixelSpace({
              x: e.points[0],
              y: e.points[1]
            });
        ...
        case RawMapEntityType.ActiveZone:
          {
            ...
            const p1 = this.convertCMCoordinatesToPixelSpace({
              x: e.points[2],
              y: e.points[3]
            });
            const p2 = this.convertCMCoordinatesToPixelSpace({
              x: e.points[4],
              y: e.points[5]
            });
            const p3 = this.convertCMCoordinatesToPixelSpace({
              x: e.points[6],
              y: e.points[7]
            });
        ...
        case RawMapLayerType.Segment:
          {
             ...
             */
            if (Math.abs(l.dimensions.x.mid - l.dimensions.x.avg) >= 8 || Math.abs(l.dimensions.y.mid - l.dimensions.y.avg) >= 8) {
              const pixels = {
                x: [],
                y: []
              };
  ...
  convertCMCoordinatesToPixelSpace(coordinates) {
    return {
      x: Math.floor(coordinates.x / this.pixelSize),
      y: Math.floor(coordinates.y / this.pixelSize)
    };
  }
