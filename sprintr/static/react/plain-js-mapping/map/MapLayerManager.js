...
export class MapLayerManager {
  constructor() {
    ...
    this.colors = {
      floor: "#0076ff",
      wall: "#333333",
      segments: ["#19A1A1", "#7AC037", "#DF5618", "#F7C841", "#9966CC" // "fallback" color
      ]
    };
  ...
  draw(data, theme) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (this.canvas.width !== Math.round(data.size.x / data.pixelSize) || this.canvas.height !== Math.round(data.size.y / data.pixelSize)) {
      ...
      if (data.layers.length > 0) {
        if (this.mapLayerManagerWorkerAvailable) {
          ...
          //Fallback if there's no worker for some reason
  ...
  getIntersectingSegment(x, y) {
    if (x < this.segmentLookupInfo.left || y < this.segmentLookupInfo.top || x > this.segmentLookupInfo.left + this.segmentLookupInfo.width || y > this.segmentLookupInfo.top + this.segmentLookupInfo.height) {
      return null;
    }
    const offset = Math.round(Math.round(x) - this.segmentLookupInfo.left + (Math.round(y) - this.segmentLookupInfo.top) * this.segmentLookupInfo.width);
