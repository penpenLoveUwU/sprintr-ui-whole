...
const ValetudoEvents = () => {
    const { data: eventData, isFetching: eventDataFetching, isLoading: eventDataLoading, error: eventDataError, refetch: eventDataRefetch, } = useValetudoEventsQuery();
    ...
    const icon = React.useMemo(() => {
        ...
        if (!eventDataLoading) {
            if (eventDataError) {
                ...
                return (<Badge badgeContent={unprocessedEventCount} color="error">
                        {icon}
                    </Badge>);
            }
        }
    ...
    const popoverContent = React.useMemo(() => {
        ...
        return (<ReloadableCard divider={false} title="Events" loading={eventDataFetching} onReload={() => {
    ...
    return (<>
            <IconButton size="large" aria-label="Events" onClick={handleMenu} color="inherit" title="Events and Notifications">
                {icon}
            </IconButton>

            <Popover open={Boolean(anchorElement)} anchorEl={anchorElement} onClose={handleClose} anchorOrigin={{
            vertical: "top",
            horizontal: "right",
        }} transformOrigin={{
            vertical: "top",
