...
const StatisticsGridItem = ({ dataPoint }) => {
    ...
    return (<>
            ...
            <Dialog open={overviewDialogOpen} onClose={() => {
            ...
            return (<Grid item xs={12} sm={4} style={{ userSelect: "none" }} key={`${dataPoint.type}_overview_${i}`}>
                                    <Card style={{ height: "100%" }}>
                                        <Grid container style={{ height: "100%" }}>
                                            <Grid item style={{
                    marginLeft: "auto",
                    marginRight: "auto"
                                                ...
                                                <CardMedia component={StatisticsAward} achievement={achievementToDisplay} achieved={achievementToDisplay === achievement}/>
                                            </Grid>
                                            <Grid item style={{ alignSelf: "flex-end", width: "100%" }}>
                                                <CardContent style={{ paddingBottom: "16px" }}>
                                                    {<Typography variant="body1" mb={2}>
...
const StatisticsAward = ({ achievement, achieved }) => {
...
const TotalStatisticsInternal = () => {
    const { data: totalStatisticsState, isLoading: totalStatisticsLoading, isError: totalStatisticsError, } = useTotalStatisticsQuery();
...
const TotalStatistics = () => {
    ...
    return (<PaperContainer>
            {supported ? <TotalStatisticsInternal /> : (<Typography color="error">This robot does not support total statistics.</Typography>)}
        </PaperContainer>);
};
const SORT_ORDER = {
    "time": 1,
