...
const VoicePackControl = () => {
    const { data: voicePack, isFetching: voicePackFetching, isError: voicePackError, refetch: voicePackRefetch, } = useVoicePackManagementStateQuery();
    ...
    const voicePackContent = React.useMemo(() => {
        ...
        return (<>
                ...
                {isError && (<Typography color="error">
                ...
                <LoadingButton loading={voicePackMutating || commandDisabled} loadingPosition="center" variant="outlined" onClick={() => {
                const command = {
                    action: "download",
                    url: url,
                    hash: hash,
    ...
    return (<CapabilityItem title="Voice packs" loading={loading} helpText={VoicepackHelp}>
            {voicePackContent}
        </CapabilityItem>);
};
