...
const SpeakerControl = () => {
    const { data: speakerVolume, isFetching: speakerVolumeLoading, isError: speakerVolumeError, } = useSpeakerVolumeStateQuery();
    ...
    const speakerVolumeContent = React.useMemo(() => {
        ...
        return (<>
                <Stack spacing={2} direction="row" sx={{ mb: 1 }} alignItems="center">
                    <VolumeDownIcon />
                    <Slider min={0} max={100} value={sliderValue} disabled={speakerVolumeLoading} onChange={onChange} onChangeCommitted={onCommit} valueLabelDisplay="auto"/>
                    <VolumeUpIcon />
                </Stack>
    ...
    return (<CapabilityItem title={"Speaker"} loading={loading}>
