...
const ManualControlInternal = () => {
    const { data: manualControlState, isLoading: manualControlStateLoading, isError: manualControlStateError, } = useManualControlStateQuery();
    const { data: manualControlProperties, isLoading: manualControlPropertiesLoading, isError: manualControlPropertiesError, } = useManualControlPropertiesQuery();
    ...
    const controls = React.useMemo(() => {
        if (manualControlPropertiesError || manualControlStateError || !manualControlProperties || !manualControlState) {
            return (<Typography color="error">Error loading manual controls</Typography>);
        }
        ...
        return (<>
                <FormControlLabel control={<Switch checked={manualControlState.enabled} disabled={loading || interacting} onChange={(e) => {
                    sendInteraction({
                        action: e.target.checked ? "enable" : "disable"
                    });
                }}/>} label="Enable manual control" style={{ marginLeft: 0 }}/>
                <Box />

                <Stack direction="row" sx={{ width: "100%", height: "30vh" }} justifyContent="center" alignItems="center">
                    <SideButton variant="outlined" disabled={!rotateCcwEnabled} onClick={() => {
                sendMoveCommand("rotate_counterclockwise");
            }}>
                        <RotateLeftIcon />
                    </SideButton>
                    <Stack sx={{ width: "40%", height: "100%", ml: 1, mr: 1 }} justifyContent="space-between">
                        <CenterButton sx={{ height: "65%" }} variant="outlined" disabled={!forwardEnabled} onClick={() => {
                sendMoveCommand("forward");
            }}>
                            <ArrowUpwardIcon />
                        </CenterButton>
                        <CenterButton sx={{ height: "30%" }} variant="outlined" disabled={!backwardEnabled} onClick={() => {
                sendMoveCommand("backward");
            }}>
                            <ArrowDownwardIcon />
                        </CenterButton>
                    </Stack>
                    <SideButton variant="outlined" disabled={!rotateCwEnabled} onClick={() => {
    ...
    return React.useMemo(() => {
        return (<FullHeightGrid container direction="column">
...
const ManualControl = () => {
    ...
    return (<PaperContainer>
            {supported ? <ManualControlInternal /> : (<Typography color="error">This robot does not support the manual control.</Typography>)}
        </PaperContainer>);
};
