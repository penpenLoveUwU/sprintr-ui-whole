...
import { Refresh as RefreshIcon, Wifi as SignalWifiUnknown, SignalWifi4Bar as SignalWifi4Bar, SignalWifi3Bar as SignalWifi3Bar, SignalWifi2Bar as SignalWifi2Bar, SignalWifi1Bar as SignalWifi1Bar, SignalWifi0Bar as SignalWifi0Bar, Visibility as VisibilityIcon, VisibilityOff as VisibilityOffIcon } from "@mui/icons-material";
...
const SignalStrengthIcon = ({ signal }) => {
...
const WifiScan = ({ onSelect }) => {
    const { data: wifiScanResult, isFetching: wifiScanFetching, refetch: triggerWifiScan } = useWifiScanQuery();
    ...
    const foundNetworkListItems = React.useMemo(() => {
        ...
        const items = deduplicatedArray.map((network) => {
            return (<ListItem key={network.ssid} sx={{
                    paddingTop: "0.25rem",
                    ...
                    onSelect(network.ssid);
                        ...
                        <ListItemText primaryTypographyProps={{ style: { fontSize: "0.95rem" } }} primary={network.ssid} secondary={`${network.signal ?? "unknown"} dBm`}/>
                    </ListItemButton>
                </ListItem>);
        });
        if (items.length > 0) {
                ...
                return [
                    ...
                    <ListItem key="more_results">
                        <ListItemButton onClick={() => {
                            ...
                            <ListItemText sx={{ textAlign: "center" }} primary="See more results"/>
                        </ListItemButton>
                    </ListItem>
                ];
            }
            ...
            return [
                <ListItem key="no_networks_found">
                    <ListItemText sx={{ textAlign: "center" }} secondary="No Wi-Fi networks found or background network scan still active"/>
                </ListItem>
            ];
        }
    ...
    return (<List sx={{
            ...
            if (!wifiScanFetching) {
                triggerWifiScan().catch(() => {
                    ...
                    <ListItemText primary={wifiScanFetching ? "Scanning..." : "Press to scan for Wi-Fi networks"}/>
                </ListItemButton>
            </ListItem>
        </List>);
};
const ProvisioningPage = () => {
    ...
    const { data: robotInformation, isLoading: robotInformationLoading, } = useRobotInformationQuery();
    const { data: version, isLoading: versionLoading, } = useValetudoVersionQuery();
    const { mutate: updateWifiConfiguration, isLoading: wifiConfigurationUpdating } = useWifiConfigurationMutation({
        onSuccess: () => {
    ...
    const robotInformationElement = React.useMemo(() => {
        ...
        return (<Grid container direction="row" sx={{ padding: "1rem", justifyContent: "space-around" }}>
                {items.map(([header, body]) => {
                return (<Grid item key={header}>
    ...
    return (<>
            ...
            <>
                ...
                <Grid item container sx={{ padding: "1rem" }} direction="column">
                    <Grid item sx={{ paddingLeft: "1rem", paddingRight: "1rem" }}>
                        <TextField label="SSID/Wi-Fi name" variant="standard" fullWidth value={newSSID} sx={{ mb: 1 }} onChange={(e) => {
            setNewSSID(e.target.value);
                    ...
                    <Grid item sx={{ paddingLeft: "1rem", paddingRight: "1rem" }}>
                        <FormControl style={{ width: "100%" }} variant="standard">
                            ...
                            <Input type={showPasswordAsPlain ? "text" : "password"} fullWidth value={newPSK} sx={{ mb: 1 }} endAdornment={<InputAdornment position="end">
            ...
            setNewPSK(e.target.value);
                    ...
                    <Grid item sx={{ marginLeft: "auto", marginTop: "0.75rem" }}>
                        <LoadingButton loading={wifiConfigurationUpdating} variant="outlined" color="success" disabled={!newSSID || !newPSK} onClick={() => {
            ...
            <ConfirmationDialog title="Apply Wi-Fi configuration?" text="" open={confirmationDialogOpen} onClose={() => {
