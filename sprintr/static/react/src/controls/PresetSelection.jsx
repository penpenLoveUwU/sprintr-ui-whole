...
const PresetSelectionControl = (props) => {
    ...
    const { data: preset } = useRobotAttributeQuery(RobotAttributeClass.PresetSelectionState, (attributes) => {
        return attributes.filter((attribute) => {
            return attribute.type === capabilityToPresetType[capability];
        })[0];
    });
    const { isLoading: presetsLoading, isError: presetLoadError, data: presets, } = usePresetSelectionsQuery(capability);
    const { mutate: selectPreset, isLoading: selectPresetIsLoading } = usePresetSelectionMutation(capability);
    ...
    const body = React.useMemo(() => {
        ...
        return (<Box px={2.5}>
                <DiscreteSlider aria-labelledby={`${capability}-slider-label`} step={null} value={sliderValue} valueLabelDisplay="off" onChange={onChange} onChangeCommitted={onCommit} min={0} max={marks.length - 1} marks={marks} track={capability !== Capability.OperationModeControl ? "normal" : false}/>
            </Box>);
    }, [
        capability,
        onChange,
    ...
    return (<Grid item>
            <Paper sx={{ minHeight: "2.5em" }}>
                <Grid container direction="column">
                    <Box px={2} pt={1}>
                        <Grid item container alignItems="center" spacing={1} onClick={() => {
            setPresetSelectionSliderOpen(!presetSelectionSliderOpen);
                            ...
                            <Grid item>
                                <LoadingFade in={selectPresetIsLoading} transitionDelay={selectPresetIsLoading ? "500ms" : "0ms"} size={20}/>
                            </Grid>
                            <Grid item sx={{
            marginLeft: "auto"
        }}>
