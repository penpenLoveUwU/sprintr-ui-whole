...
const Dock = () => {
    ...
    const { data: dockStatus, isLoading: isDockStatusLoading, } = useRobotAttributeQuery(RobotAttributeClass.DockStatusState);
    const { data: attachments, isLoading: isAttachmentLoading, } = useRobotAttributeQuery(RobotAttributeClass.AttachmentState);
    ...
    const { mutate: triggerDockEmpty, isLoading: emptyIsExecuting, } = useAutoEmptyDockManualTriggerMutation();
    const { mutate: triggerMopDockCleanCommand, isLoading: mopDockCleanCommandExecuting, } = useMopDockCleanManualTriggerMutation();
    const { mutate: triggerMopDockDryCommand, isLoading: mopDockDryCommandExecuting, } = useMopDockDryManualTriggerMutation();
    ...
    return (<Grid item>
            ...
            <Grid item xs>
                                    <Button disabled={commandIsExecuting || !["idle", "cleaning", "pause"].includes(dockState) || robotState !== "docked" || !mopAttachmentAttached} variant="outlined" size="medium" color="inherit" onClick={() => {
            ...
            <Grid item xs>
                                    <Button disabled={commandIsExecuting || !["idle", "drying", "pause"].includes(dockState) || robotState !== "docked" || !mopAttachmentAttached} variant="outlined" size="medium" color="inherit" onClick={() => {
            ...
            <Grid item xs>
                                    <Button disabled={commandIsExecuting || robotState !== "docked"} variant="outlined" size="medium" color="inherit" onClick={() => {
                    triggerDockEmpty();
                }} sx={{ width: "100%" }}>
