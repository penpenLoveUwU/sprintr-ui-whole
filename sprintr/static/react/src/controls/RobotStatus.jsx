...
const RobotStatus = () => {
    const { data: status, isLoading: isStatusLoading, isError: isStatusError, } = useRobotStatusQuery();
    const { data: batteries, isLoading: isBatteryLoading, isError: isBatteryError, } = useRobotAttributeQuery(RobotAttributeClass.BatteryState);
    ...
    const stateDetails = React.useMemo(() => {
        ...
        return (<Typography variant="overline" color="textSecondary">
                {status.value}
    ...
    const batteriesDetails = React.useMemo(() => {
        ...
        return batteries.map((battery, index) => {
            return (<Grid container key={index.toString()} direction="column" spacing={1}>
                ...
                <Box p={1}>
                    <Grid container spacing={2} direction="column">
                        <Grid item container>
                            ...
                            {batteries !== undefined && batteries.length > 0 && (<Grid item xs container direction="column" sx={{ paddingRight: "8px" }}>
                                    <Grid item>
