...
const MQTTStatusComponent = ({ status, statusLoading, statusError }) => {
    ...
    const getContentForState = () => {
        switch (status.state) {
            case "disconnected":
                return (<Typography variant="h5">Disconnected</Typography>);
            case "ready":
                return (<Typography variant="h5">Connected</Typography>);
            case "init":
                return (<Typography variant="h5">Connecting/Reconfiguring</Typography>);
            case "lost":
            case "alert":
                return (<Typography variant="h5">Connection error</Typography>);
        }
    };
    ...
    return (<>
            ...
            userSelect: "none"
                ...
                <Grid item container direction="row" style={{ marginTop: "1rem" }}>
                    <Grid item style={{ flexGrow: 1 }} p={1}>
                        <Card sx={{ boxShadow: 3 }}>
                            <CardContent>
...
const GroupBox = (props) => {
    let title = (<Typography variant="subtitle1" sx={{
            marginBottom: 0,
        }}>
            {props.title}
        </Typography>);
    if (props.onChange) {
        title = (<FormControlLabel control={<Checkbox checked={props.checked} disabled={props.disabled} onChange={props.onChange}/>} disableTypography label={title}/>);
    }
    return (<Container sx={{ m: 0.2 }}>
            {title}
...
const MQTTInput = ({ mqttConfiguration, modifyMQTTConfig, disabled, title, helperText, required, configPath, additionalProps, inputPostProcessor }) => {
    ...
    return (<FormControl required={required} error={error} component="fieldset" sx={{ ml: 1, mt: 2 }} disabled={disabled}>
            <InputLabel htmlFor={inputId} disabled={disabled}>{title}</InputLabel>
            <Input id={inputId} value={value} onChange={(e) => {
        ...
        }} aria-describedby={helperId} disabled={disabled} {...additionalProps}/>
            <FormHelperText id={helperId} disabled={disabled}>
                {helperText}
            </FormHelperText>
        </FormControl>);
};
const MQTTSwitch = ({ mqttConfiguration, modifyMQTTConfig, disabled, title, configPath, }) => {
    const value = getIn(mqttConfiguration, configPath);
    return (<FormControlLabel control={<Switch checked={value} onChange={(e) => {
                modifyMQTTConfig(e.target.checked, configPath);
            }}/>} disabled={disabled} label={title}/>);
};
const MQTTOptionalExposedCapabilitiesEditor = ({ mqttConfiguration, modifyMQTTConfig, disabled, configPath, exposableCapabilities }) => {
    let selection = getIn(mqttConfiguration, configPath);
    return (<Container sx={{ m: 0.2 }}>
            <FormGroup>
                {exposableCapabilities.map((capabilityName) => {
            return (<FormControlLabel key={capabilityName} control={<Checkbox checked={selection.includes(capabilityName)} onChange={(e) => {
                        if (e.target.checked) {
                    ...
                    }}/>} disabled={disabled} label={capabilityName}/>);
        })}
...
const MQTTConnectivity = () => {
    ...
    const { data: storedMQTTConfiguration, isLoading: mqttConfigurationLoading, isError: mqttConfigurationError, } = useMQTTConfigurationQuery();
    const { data: mqttStatus, isLoading: mqttStatusLoading, isFetching: mqttStatusFetching, isError: mqttStatusError, refetch: refetchMqttStatus, } = useMQTTStatusQuery();
    const { data: mqttProperties, isLoading: mqttPropertiesLoading, isError: mqttPropertiesError } = useMQTTPropertiesQuery();
    ...
    return (<PaperContainer>
            <Grid container direction="row">
                <Box style={{ width: "100%" }}>
                    <DetailPageHeaderRow title="MQTT Connectivity" icon={<MQTTIcon />} onRefreshClick={() => {
            refetchMqttStatus().catch(() => {
                /* intentional */
            });
        }} isRefreshing={mqttStatusFetching}/>

                    <MQTTStatusComponent status={mqttStatus} statusLoading={mqttStatusLoading} statusError={mqttStatusError}/>
                ...
                modifyMQTTConfig(e.target.checked, ["enabled"]);
                    ...
                    <GroupBox title="Connection">
                        <MQTTInput mqttConfiguration={mqttConfiguration} modifyMQTTConfig={modifyMQTTConfig} disabled={disabled} title="Host" helperText="The MQTT Broker hostname" required={true} configPath={["connection", "host"]}/>
                        <MQTTInput mqttConfiguration={mqttConfiguration} modifyMQTTConfig={modifyMQTTConfig} disabled={disabled} title="Port" helperText="The MQTT Broker port" required={true} configPath={["connection", "port"]} additionalProps={{ type: "number" }}/>

                        <GroupBox title="TLS" checked={mqttConfiguration.connection.tls.enabled} disabled={disabled} onChange={(e) => {
            modifyMQTTConfig(e.target.checked, ["connection", "tls", "enabled"]);
        }}>
                            <MQTTInput mqttConfiguration={mqttConfiguration} modifyMQTTConfig={modifyMQTTConfig} disabled={disabled} title="CA" helperText="The optional Certificate Authority to verify the connection with" required={false} configPath={["connection", "tls", "ca"]} additionalProps={{
            multiline: true,
            minRows: 3,
            maxRows: 10,
        }}/>
                        ...
                        <GroupBox title="Authentication">
                            <GroupBox title="Credentials" disabled={disabled} checked={mqttConfiguration.connection.authentication.credentials.enabled} onChange={(e) => {
            modifyMQTTConfig(e.target.checked, ["connection", "authentication", "credentials", "enabled"]);
        }}>
                                <MQTTInput mqttConfiguration={mqttConfiguration} modifyMQTTConfig={modifyMQTTConfig} disabled={disabled} title="Username" helperText="Username for authentication" required={true} configPath={["connection", "authentication", "credentials", "username"]}/>
                                <MQTTInput mqttConfiguration={mqttConfiguration} modifyMQTTConfig={modifyMQTTConfig} disabled={disabled} title="Password" helperText="Password for authentication" required={false} configPath={["connection", "authentication", "credentials", "password"]} additionalProps={{
            ...
            endAdornment: (<InputAdornment position="end">
                    ...
                    e.preventDefault();
                            ...
                            <GroupBox title="Client certificate" disabled={disabled} checked={mqttConfiguration.connection.authentication.clientCertificate.enabled} onChange={(e) => {
            modifyMQTTConfig(e.target.checked, ["connection", "authentication", "clientCertificate", "enabled"]);
        }}>

                                <MQTTInput mqttConfiguration={mqttConfiguration} modifyMQTTConfig={modifyMQTTConfig} disabled={disabled} title="Certificate" helperText="The full certificate as a multi-line string" required={true} configPath={["connection", "authentication", "clientCertificate", "certificate"]} additionalProps={{
            multiline: true,
            minRows: 3,
            maxRows: 10
        }}/>
                                <MQTTInput mqttConfiguration={mqttConfiguration} modifyMQTTConfig={modifyMQTTConfig} disabled={disabled} title="Key" helperText="The full key as a multi-line string" required={true} configPath={["connection", "authentication", "clientCertificate", "key"]} additionalProps={{
            multiline: true,
            minRows: 3,
            maxRows: 10
        }}/>
                    ...
                    <GroupBox title="Customizations" disabled={disabled}>
                        <MQTTInput mqttConfiguration={mqttConfiguration} modifyMQTTConfig={modifyMQTTConfig} disabled={disabled} title="Topic prefix" helperText="MQTT topic prefix" required={false} configPath={["customizations", "topicPrefix"]} additionalProps={{
            ...
            return sanitizeStringForMQTT(value, true).replace(/\/\//g, "/");
                        ...
                        <MQTTInput mqttConfiguration={mqttConfiguration} modifyMQTTConfig={modifyMQTTConfig} disabled={disabled} title="Identifier" helperText="The machine-readable name of the robot" required={false} configPath={["identity", "identifier"]} additionalProps={{
            ...
            color: theme.palette.secondary.main
                        ...
                        <MQTTSwitch mqttConfiguration={mqttConfiguration} modifyMQTTConfig={modifyMQTTConfig} disabled={disabled} title="Provide map data" configPath={["customizations", "provideMapData"]}/>
                    </GroupBox>

                    <GroupBox title="Interfaces" disabled={disabled}>
                        <GroupBox title="Homie" checked={mqttConfiguration.interfaces.homie.enabled} disabled={disabled} onChange={(e) => {
            modifyMQTTConfig(e.target.checked, ["interfaces", "homie", "enabled"]);
                            ...
                            <FormControl component="fieldset" variant="standard">
                                ...
                                <FormGroup>
                                    <MQTTSwitch mqttConfiguration={mqttConfiguration} modifyMQTTConfig={modifyMQTTConfig} disabled={disabled} title={"Provide autodiscovery for \"I Can't Believe It's Not Valetudo\" map"} configPath={["interfaces", "homie", "addICBINVMapProperty"]}/>
                                    <MQTTSwitch mqttConfiguration={mqttConfiguration} modifyMQTTConfig={modifyMQTTConfig} disabled={disabled} title="Delete autodiscovery on shutdown" configPath={["interfaces", "homie", "cleanAttributesOnShutdown"]}/>
                                </FormGroup>
                            </FormControl>
                        </GroupBox>

                        <GroupBox title="Home Assistant" checked={mqttConfiguration.interfaces.homeassistant.enabled} disabled={disabled} onChange={(e) => {
            modifyMQTTConfig(e.target.checked, ["interfaces", "homeassistant", "enabled"]);
                            ...
                            <FormControl component="fieldset" variant="standard">
                                ...
                                <FormGroup>
                                    <MQTTSwitch mqttConfiguration={mqttConfiguration} modifyMQTTConfig={modifyMQTTConfig} disabled={disabled} title="Delete autodiscovery on shutdown" configPath={["interfaces", "homeassistant", "cleanAutoconfOnShutdown"]}/>
                                </FormGroup>
                            </FormControl>
                        </GroupBox>
                    </GroupBox>
            ...
            <GroupBox title="Optional exposable capabilities" disabled={disabled}>
                            <MQTTOptionalExposedCapabilitiesEditor mqttConfiguration={mqttConfiguration} modifyMQTTConfig={modifyMQTTConfig} disabled={disabled} configPath={["optionalExposedCapabilities"]} exposableCapabilities={mqttProperties.optionalExposableCapabilities}/>
                        </GroupBox>}
            ...
            marginBottom: "2rem"
                    ...
                    <Grid container>
                        <Grid item style={{ marginLeft: "auto" }}>
                            <LoadingButton disabled={!configurationModified} loading={mqttConfigurationUpdating} color="primary" variant="outlined" onClick={() => {
