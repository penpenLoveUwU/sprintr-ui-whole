...
const WifiStatusComponent = ({ status, statusLoading, statusError }) => {
    ...
    const getContentForState = () => {
        switch (status.state) {
            case "not_connected":
                return (<Typography variant="h5">Not connected</Typography>);
            case "unknown":
                return (<Typography variant="h5">Unknown</Typography>);
            case "connected":
                return (<>
                        ...
                        <Typography variant="subtitle2" style={{ marginTop: "0.5rem", color: theme.palette.grey[theme.palette.mode === "light" ? 400 : 700] }}>
...
const WifiConnectivity = () => {
    const { data: wifiStatus, isLoading: wifiStatusLoading, isFetching: wifiStatusFetching, isError: wifiStatusLoadError, refetch: refetchWifiStatus, } = useWifiStatusQuery();
    const { data: properties, isLoading: propertiesLoading, isError: propertiesLoadError } = useWifiConfigurationPropertiesQuery();
    ...
    return (<PaperContainer>
            <Grid container direction="row">
                <Box style={{ width: "100%" }}>
                    <DetailPageHeaderRow title="Wi-Fi Connectivity" icon={<WifiIcon />} onRefreshClick={() => {
            refetchWifiStatus().catch(() => {
                /* intentional */
            });
        }} isRefreshing={wifiStatusFetching}/>

                    <WifiStatusComponent status={wifiStatus} statusLoading={wifiStatusLoading} statusError={wifiStatusLoadError}/>
            ...
            <Grid container spacing={1} sx={{ mb: 1 }} direction="row">
                            <Grid item xs="auto" style={{ flexGrow: 1 }}>
                                <TextField style={{ width: "100%" }} label="SSID/Wi-Fi name" value={newSSID} variant="standard" onChange={e => {
            ...
            <Grid container>
                            <Grid item style={{ marginLeft: "auto" }}>
                                <LoadingButton loading={configurationUpdating} color="primary" variant="outlined" disabled={!(configurationModified && newSSID && newPSK)} onClick={() => {
            ...
            <ConfirmationDialog title="Apply new Wi-Fi configuration?" text="" open={confirmationDialogOpen} onClose={() => {
