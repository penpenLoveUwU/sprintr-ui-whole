...
const NetworkAdvertisementSettings = () => {
    const { data: storedConfiguration, isLoading: configurationLoading, isError: configurationError, } = useNetworkAdvertisementConfigurationQuery();
    const { data: properties, isLoading: propertiesLoading, isError: propertiesLoadError } = useNetworkAdvertisementPropertiesQuery();
    ...
    return (<PaperContainer>
            <Grid container direction="row">
                <Box style={{ width: "100%" }}>
                    <DetailPageHeaderRow title="Network Advertisement" icon={<NetworkAdvertisementIcon />}/>

                    <FormControlLabel control={<Checkbox checked={enabled} onChange={e => {
                setEnabled(e.target.checked);
                setConfigurationModified(true);
            }}/>} label="Network Advertisement enabled" sx={{ mb: 1, marginTop: "1rem", userSelect: "none" }}/>
                    <Grid container spacing={1} sx={{ mb: 1, mt: "1rem" }} direction="row">
                        <Grid item xs="auto" style={{ flexGrow: 1 }}>
                            <TextField style={{ width: "100%" }} label="Zeroconf Hostname" value={properties?.zeroconfHostname ?? ""} variant="standard" disabled={true} InputProps={{
            readOnly: true,
        }}/>
                        </Grid>
                    </Grid>
