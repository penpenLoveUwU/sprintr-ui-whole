...
const ConfigRestoreButtonListMenuItem = () => {
    const { mutate: restoreDefaultConfiguration, isLoading: restoreDefaultConfigurationIsExecuting } = useRestoreDefaultConfigurationMutation();
    return (<ButtonListMenuItem primaryLabel="Restore Default Configuration" secondaryLabel="This will only affect Valetudo" icon={<ConfigRestoreIcon />} buttonLabel="Go" buttonColor={"error"} confirmationDialog={{
...
const FriendlyNameEditModalListMenuItem = () => {
    const { data: valetudoCustomizations, isLoading: valetudoCustomizationsLoading, } = useValetudoCustomizationsQuery();
    const { mutate: updateValetudoCustomizations, isLoading: valetudoCustomizationsUpdating } = useValetudoCustomizationsMutation();
    ...
    return (<TextEditModalListMenuItem isLoading={valetudoCustomizationsLoading || valetudoCustomizationsUpdating} value={valetudoCustomizations?.friendlyName ?? ""} dialog={{
        ...
        }} icon={<FriendlyNameIcon />} primaryLabel={"Custom Friendly Name"} secondaryLabel={secondaryLabel}/>);
};
...
const UpdateProviderSelectListMenuItem = () => {
    const { data: storedConfiguration, isLoading: configurationLoading, isError: configurationError, } = useUpdaterConfigurationQuery();
    ...
    return (<SelectListMenuItem options={updateProviders} currentValue={currentValue} setValue={(e) => {
            updateConfiguration({
                updateProvider: e.value
            });
        }} disabled={disabled} loadError={configurationError} primaryLabel="Update Channel" secondaryLabel="Select the channel used by the inbuilt updater" icon={<UpdaterIcon />}/>);
};
const ValetudoOptions = () => {
    ...
    return (<PaperContainer>
            <ListMenu primaryHeader={"Valetudo Options"} secondaryHeader={"Tunables and actions provided by Valetudo"} listItems={listItems}/>
        </PaperContainer>);
};
