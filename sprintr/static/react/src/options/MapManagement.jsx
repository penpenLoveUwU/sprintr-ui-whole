...
export const MappingPassButtonItem = () => {
    ...
    return (<ButtonListMenuItem primaryLabel="Mapping Pass" secondaryLabel="Create a new map" icon={<MappingPassIcon />} buttonLabel="Go" confirmationDialog={{
            title: "Start mapping pass?",
            body: "Do you really want to start a mapping pass?"
        }} action={startMappingPass} actionLoading={mappingPassStarting}/>);
};
const MapResetButtonItem = () => {
    const { mutate: resetMap, isLoading: mapResetting } = useMapResetMutation();
    return (<ButtonListMenuItem primaryLabel="Map Reset" secondaryLabel="Delete the current map" icon={<MapResetIcon />} buttonLabel="Go" buttonColor={"error"} confirmationDialog={{
            title: "Reset map?",
            body: "Do you really want to reset the map?"
        }} action={resetMap} actionLoading={mapResetting}/>);
};
export const PersistentMapSwitchListItem = () => {
    const [dialogOpen, setDialogOpen] = React.useState(false);
    const { data: persistentData, isFetching: persistentDataLoading, isError: persistentDataError, } = usePersistentDataQuery();
    ...
    return (<>
            <ToggleSwitchListMenuItem value={persistentData?.enabled ?? false} setValue={(value) => {
        ...
        }} disabled={disabled} loadError={persistentDataError} primaryLabel={"Persistent maps"} secondaryLabel={"Store a persistent map"} icon={<PersistentMapControlIcon />}/>
            <ConfirmationDialog title="Disable persistent maps?" text={(<>
...
const ValetudoMapDataExportButtonItem = () => {
    ...
    const { data: mapData, isLoading: mapIsLoading, } = useRobotMapQuery();
    return (<ButtonListMenuItem primaryLabel="Export ValetudoMap" secondaryLabel="Download a ValetudoMap data export to use with other tools" icon={<ValetudoMapDownloadIcon />} buttonLabel="Go" action={() => {
            if (valetudoInformation && mapData) {
                const timestamp = new Date().toISOString().replaceAll(":", "-").split(".")[0];
                const mapExportBlob = new Blob([JSON.stringify(mapData, null, 2)], { type: "application/json" });
...
const MapManagement = () => {
    ...
    const robotManagedListItems = React.useMemo(() => {
        ...
        if (persistentMapControlCapabilitySupported ||
            ...
            if (persistentMapControlCapabilitySupported) {
                items.push(<PersistentMapSwitchListItem key="persistentMapSwitch"/>);
            }
            if (mappingPassCapabilitySupported) {
                items.push(<MappingPassButtonItem key="mappingPass"/>);
            }
            if (mapResetCapabilitySupported) {
                items.push(<MapResetButtonItem key="mapReset"/>);
            }
        ...
        if (mapSegmentEditCapabilitySupported || mapSegmentRenameCapabilitySupported) {
            items.push(<LinkListMenuItem key="segmentManagement" url="/options/map_management/segments" primaryLabel="Segment Management" secondaryLabel="Modify the maps segments" icon={<SegmentEditIcon />}/>);
        }
        if (combinedVirtualRestrictionsCapabilitySupported) {
            items.push(<LinkListMenuItem key="virtualRestrictionManagement" url="/options/map_management/virtual_restrictions" primaryLabel="Virtual Restriction Management" secondaryLabel="Create, modify and delete various virtual restrictions" icon={<VirtualRestrictionsIcon />}/>);
        }
        return items;
    }, [
    ...
    const utilityMapItems = React.useMemo(() => {
        return [
            <LinkListMenuItem key="robotCoverageMap" url="/options/map_management/robot_coverage" primaryLabel="Robot Coverage Map" secondaryLabel="Check the robots coverage" icon={<CleanupCoverageIcon />}/>,
            <ValetudoMapDataExportButtonItem key="valetudoMapDataExport"/>
        ];
    }, []);
    return (<PaperContainer>
            <ListMenu primaryHeader={"Robot-managed Map Features"} secondaryHeader={"These features are managed and provided by the robot's firmware"} listItems={robotManagedListItems} helpText={MapManagementHelp}/>
            <ListMenu primaryHeader={"Map Utilities"} secondaryHeader={"Do neat things with the map"} listItems={utilityMapItems} helpText={MapUtilitiesHelp} style={{ marginTop: "1rem" }}/>
        </PaperContainer>);
};
